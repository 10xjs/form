 ...
 * @typeParam WS Type of form warning state.
 */
export interface FormOptions<VS, SD, ES = null, WS = null> {
  /**
  ...
  onSubmit: (
    values: VS,
  ) => undefined | SubmitResult<SD> | Promise<SubmitResult<SD>>;
   ...
   * ```
   */
  warn?: (values: VS) => null | WS;
}

/** @internal */
class FormConfig<VS, SD, ES = null, WS = null> {
  ...
  get validate() {
    return this.config.validate ?? (() => null);
  }

  get warn() {
    return this.config.warn ?? (() => null);
  }
}
...
> {
  ...
  constructor(initialValues: VS, options: FormOptions<VS, SD, ES, WS>) {
    ...
    super({
      ...
      errors: normalizeError(config.validate(initialValues)),
      warnings: normalizeError(config.warn(initialValues)),
   ...
   */
  submit() {
    return new Promise<undefined | SubmitResult<SD>>((resolve) => {
